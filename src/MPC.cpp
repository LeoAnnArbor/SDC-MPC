...
// variables in a vector. To better access each part, we first determine 
// the start position of one variable .
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // determine total cost based on state.
    for (int i = 0; i < N; i++) {
      // trajectory
...
Solution MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  // to max neg and pos calues
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] = 1.0e19;   
  }

  // constaint for delta is set to be within -25 to 25 degree
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;     
  }

  // constaint delta to be the previous control with specified latency time
  for (int i = delta_start; i < delta_start + latency_ind; i++) {
    vars_lowerbound[i] = delta_prev;
    vars_upperbound[i] = delta_prev;     
  }

  // constaint for acc is set to be within -1 to 1 degree
